...
class Trainer():
    def __init__(self, max_num_epochs, criteria, optimizer, eval_metric, eval_interval=1, device='cpu', verbose=3,
                 early_stop=None):
...
class ClassifierTrainer(Trainer):
        ...
        for source_batch, terminal_batch, labels_batch, pair_batch, pair_source_type, pair_degree in train_loader:
            ...
            if self.intermediate_loss_weight:
                intermediate_loss = self.intermediate_criteria(torch.squeeze(pre_pred),
                                                               torch.repeat_interleave(labels_batch,
                                                                                       model.n_experiments).to(
                                                                   torch.get_default_dtype())) / model.n_experiments
    ...
    def eval(self, model, eval_loader, output_probs=False):
        """
        Evaluation method as used in training process.
        :param model: DeepClasifier model.
        :param eval_loader: dataloader.
        :param output_probs: whether to output raw output probs
        :return:
        """
        ...
        model.eval()
        with torch.no_grad():
            for eval_source_batch, eval_terminal_batch, eval_labels_batch, \
                eval_pair_batch, eval_pair_source_type_batch, eval_pair_degree_batch in eval_loader:
                if self.device != 'cpu':
                ...
                if self.intermediate_loss_weight:
                    eval_intermediate_loss = self.intermediate_criteria(torch.squeeze(pre_pred),
                                                                        torch.repeat_interleave(eval_labels_batch,
                                                                                                model.n_experiments).to(
                                                                            torch.get_default_dtype())) / model.n_experiments
        ...
        if output_probs:
            ...
            return avg_eval_loss, avg_eval_intermediate_loss, avg_eval_classifier_loss, eval_acc, mean_auc, precision, \
                   recall
    ...
    def eval_by_source(self, model, eval_loader, output_probs=False):
        """
        Evaluate dataset, includes source type stratification.
        :param model: DeepClassifier model.
        :param eval_loader: dataloader.
        :param output_probs: wheter to return raw output probs
        :return:
        """
        type_output_dict = {'probs': [], 'labels': [], 'intermediate_loss': 0, 'classifier_loss': 0, 'loss': 0,
                            'hits': 0}
        type_result_dict = {'mean_auc': 0, 'avg_loss': 0, 'avg_classifier_loss': 0, 'avg_intermediate_loss': 0,
                            'acc': 0}
        ...
        with torch.no_grad():
            for eval_source_batch, eval_terminal_batch, eval_labels_batch, eval_pair_batch, \
                ...
                for unique_type in unique_types:
                    ...
                    if self.intermediate_loss_weight:
                        eval_intermediate_loss =\
                            self.intermediate_criteria(torch.squeeze(pre_pred, dim=1),
                                                       torch.repeat_interleave(type_labels,
                                                                               model.n_experiments)
                                                       .to(torch.get_default_dtype())) / model.n_experiments
            ...
            for unique_type in output_per_type_dict.keys():
                output_per_type_dict[unique_type]['probs'] = \
                    torch.nn.functional.softmax(
                        torch.squeeze(Tensor(np.concatenate(output_per_type_dict[unique_type]['probs'], 0))), dim=1)
        ...
        if output_probs:
            ...
            for unique_type in output_per_type_dict.keys():
                ...
                results_per_type_dict[unique_type]['avg_loss'] =\
                    output_per_type_dict[unique_type]['loss'] / n_type_samples
                results_per_type_dict[unique_type]['avg_intermediate_loss'] =\
                    output_per_type_dict[unique_type]['intermediate_loss'] / n_type_samples
                results_per_type_dict[unique_type]['avg_classifier_loss'] =\
                    output_per_type_dict[unique_type]['classifier_loss'] / n_type_samples
